...
leaves out the (technically optional) GameData parent directory.
...
\paragraph*{Base Path}
Point this not at ``GameData'' itself, but at the mod folder within it (e.g., ``C:/MyGame/GameData/base'').
This is optional if the exported file's path already contains ``GameData''.
...
When adding new animations to an existing skeleton, select the skeleton file to match so the exporter remaps
bone indices by name (for instance, reference ``models/players/\_humanoid/\_humanoid.gla'' if that is your rig).
