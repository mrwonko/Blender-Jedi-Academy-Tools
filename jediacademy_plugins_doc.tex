...
\begin{document}
 ...
 \begin{itemize}
  ...
  read or written on Blender 5.0 and newer.
 ...
 This addon requires Blender 4.1 or newer, and has been tested up to Blender 5.2.
...
\paragraph*{Skin}
Enter the suffix of the skin file whose textures the GLM should use, for example type ``blue'' to load
``model\_blue.skin'' alongside ``model.glm''.

\paragraph*{Guess Textures}
Tick this if the GLM stored corrupted texture paths; the importer will try to reconstruct the missing first
character (useful when you do not know or do not want to specify a skin manually).

\paragraph*{Base Path}
Point this not at ``GameData'' itself, but at the mod folder within it (e.g., ``C:/MyGame/GameData/base''). This is
optional if the opened file's path already contains ``GameData'', since the importer can deduce it automatically;
it's mainly needed for users who don't work directly within the game directory, or whose game directory setup
leaves out the (technically optional) GameData parent directory. Used to resolve relative paths, like the .gla override.

\paragraph*{.gla override}
Supply another GLA path in Jedi Academy base-relative notation, for example
``models/players/\_humanoid\_academy1/\_humanoid\_academy1.gla'' (which contains additional animations specific
to the academy1 level), to make the GLM load a different skeleton. This will only work if the bone order also
matches.

\paragraph*{Scale}
Use this to shrink or enlarge the imported scene\_root so the model fits Blender's unit system (e.g., enter
``0.01'' for a model that would otherwise show up enormous); the exporter ignores the value because everything
is saved relative to scene\_root.

\paragraph*{Skeleton changes}
Pick ``Jedi Academy \_humanoid'' to let the addon reorganize the odd default hierarchy (the lower arms are
normally not children of the upper arms) into something easier to animate while keeping exports working, as long
as the ``gla reference'' export setting is set correctly.

\paragraph*{Animations}
Choose whether to import no animation, the entire file, or just a range; selecting ``Range'' keeps Blender from
loading all 20,000+ frames when you're only interested in a part of the sequence.

\paragraph*{Start frame}
When importing a range, set this to the zero-based frame where you want to begin (e.g., ``100'' to start at frame 100).

\paragraph*{Number of frames}
When importing a range, enter how many frames to pull in (for example ``250'' to import 250 frames starting at the
start frame).
...
\paragraph*{Base Path}
Point this not at ``GameData'' itself, but at the mod folder within it (e.g., ``C:/MyGame/GameData/base'').
This is optional if the exported file's path already contains ``GameData''. It's only needed so the relative
``.gla name'' below can be resolved into an actual file, which needs to be loaded during export.

\paragraph*{.gla name}
Tell the exporter which skeleton this model uses because GLM files store bone indices, not names. Enter a
base-relative path like ``models/players/\_humanoid/\_humanoid.gla'', or leave it blank for models such as
weapons that do not use a skeleton.
...
\paragraph*{Base Path}
Point this not at ``GameData'' itself, but at the mod folder within it (e.g., ``C:/MyGame/GameData/base'').
This is optional if the exported file's path already contains ``GameData''. Used to automatically derive
the relative gla name, and to resolve the relative gla reference (if any).

\paragraph*{gla name}
Type the filename you are exporting (for example ``models/players/\_humanoid/\_humanoid.gla''); you can leave it
empty if the exporter can guess it from the base path.

\paragraph*{gla reference}
When adding new animations to an existing skeleton, select the skeleton file to match so the exporter can match
the bone order (for instance, reference ``models/players/\_humanoid/\_humanoid.gla'' if that is your rig).
Without this, the exported skeleton will be incompatible with any existing models that reference the skeleton
it replaces. Leave this empty only when creating a new skeleton with no existing models.
